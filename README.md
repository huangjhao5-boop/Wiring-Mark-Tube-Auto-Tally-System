...
### 5. 🔢 線號自動連號遞增模式 (+1 Auto Increment - 支援數字與英文字母)
- 開啟「連番自動インクリメント」後，每確認一條配線，系統自動計算下一個初始線號：
  - **純數字與補零**：`101` $\to$ `102` $\to$ `103`、`001` $\to$ `002`。
  - **英文字母**：`A` $\to$ `B` $\to$ `C` ... `Z` $\to$ `AA` $\to$ `AB`（支援大寫與小寫）。
  - **數字 + 字母後綴**：`101A` $\to$ `101B` $\to$ `101C` ... `101Z` $\to$ `101AA`。
  - **自訂前綴**：`TB-A` $\to$ `TB-B`、`CN1-01` $\to$ `CN1-02`。連續作圖效率提升數倍。
